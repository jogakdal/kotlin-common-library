...
```
> version 기본값은 `1.0`, datetime 은 ISO-8601 UTC(Z) 포맷.
...
class BuildExample {
    void buildSamples() {
        ...
        StandardResponse<StatusPayload> timedResp = StandardResponse.buildWithCallback(
...
| 상황 | 점검 포인트 | 해결 |
|------|-------------|------|
| total < current | 잘못된 총계 계산 | 총계 재계산 또는 total=0 정책 → 문서화 |
