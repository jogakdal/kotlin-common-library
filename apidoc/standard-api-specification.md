...
**원칙 1 — 오류 상황에서 2xx 응답 금지**

오류를 응답해야 하는 상황에서는 절대 2xx 상태 코드를 사용하지 않습니다. 클라이언트 요청이 잘못되었거나, 권한이 없거나, 리소스를 찾을 수 없거나, 서버가 처리할 수 없는 상태라면 반드시 4xx 또는 5xx 상태 코드를 반환해야 합니다.

단일 리소스를 응답하는 API에서 응답할 페이로드가 없는 경우에도 빈 payload로 2xx를 응답하지 않고, 적절한 오류 응답(4xx 또는 5xx)을 반환해야 합니다.

**원칙 2 — 상태 코드별 Payload 스키마 및 `status` 필드 규칙**
...
| **204**          | 응답 본문이 없습니다.                                                                                       | —                                                                                   |
| **4xx / 5xx**    | 오류 정보를 담는 스키마([오류 응답 형식](#오류실패-응답-형식) 참조)를 포함할 것을 권고합니다.                                           | `FAILURE`로 명시할 것을 권고합니다.                                                            |
...
`status` 필드의 표준값은 `SUCCESS` / `FAILURE` 이며, 실패 상황에서는 `status="FAILURE"` 로 표준화합니다. 2xx 응답에서는 `SUCCESS` 외에 상황에 따라 다른 값(`SUSPENDED`, `DEPRECATED`, `STREAMING` 등)을 활용할 수 있습니다. (상세: [상태 코드와 응답 본문의 일관성](#상태-코드와-응답-본문의-일관성) 참조)
>```
>{
>  "status": "FAILURE",
>  "version": <버전 정보>,
...
오류 코드(`code`)는 각 서비스 또는 도메인별로 정의하여 문서화해야 합니다.
