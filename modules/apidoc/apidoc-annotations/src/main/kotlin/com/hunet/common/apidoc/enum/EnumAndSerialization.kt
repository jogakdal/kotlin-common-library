...
interface DescriptiveEnum {
    ...
    companion object {
        const val OPEN_DELIMITER = "{$}"
        const val CLOSE_DELIMITER = "{$}"
        const val DESCRIPTION_MARKER = "${OPEN_DELIMITER}DESCRIPTION$CLOSE_DELIMITER" // VariableProcessor 구분자와 맞춤
        ...
        fun replaceDescription(description: String, type: KClass<out DescriptiveEnum>): String {
            if (!description.contains(DESCRIPTION_MARKER)) return description
            val constants = type.java.enumConstants ?: return description
            val registry = object : VariableResolverRegistry {
                override val resolvers: Map<String, (List<Any?>) -> Any> = mapOf(
                    "DESCRIPTION" to { _: List<Any?> -> toDescription(constants.filterIsInstance<DescriptiveEnum>().toTypedArray()) }
                )
            }
            val processor = VariableProcessor(listOf(registry))
            return processor.process(description, VariableProcessor.Delimiters(OPEN_DELIMITER, CLOSE_DELIMITER))
        }
    }
}
