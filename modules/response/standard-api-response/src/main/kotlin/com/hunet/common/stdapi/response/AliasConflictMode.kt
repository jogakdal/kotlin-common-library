...
class AliasConflictModeConfiguration(
    ...
    @PostConstruct
    fun init() {
        val resolvedMode = runCatching { AliasConflictMode.valueOf(configuredMode.trim().uppercase()) }
            .getOrElse { AliasConflictMode.WARN }
        AliasConflictConfig.mode = resolvedMode

        val resolvedRes = runCatching { AliasConflictResolution.valueOf(configuredResolution.trim().uppercase()) }
            .getOrElse { AliasConflictResolution.FIRST_WIN }
