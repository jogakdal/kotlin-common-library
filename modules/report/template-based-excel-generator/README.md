...
- **필드 숨기기**: repeat 영역 내 특정 필드(컬럼)를 동적으로 숨김 처리. 삭제(DELETE) 또는 비활성화(DIM) 모드 선택 가능
...
변수 치환, 이미지 삽입, 반복 데이터 확장, 자동 셀 병합, 요소 묶음, 필드 숨기기, 수식 범위 조정, 조건부 서식 복제, 차트 데이터 반영까지 TBEG이 자동으로 처리합니다.
...
> `generate()` 메서드가 `ByteArray`를 반환하므로 웹 응답, 파일 저장, 스트림 전송 등 어디에든 바로 전달할 수 있고,
> 스트리밍 모드 덕분에 대용량 데이터도 메모리 부담 없이 처리할 수 있습니다.
...
| `${hideable(value=item.필드, ...)}` | 필드 숨기기 | `${hideable(value=emp.salary, bundle=C1:C3, mode=dim)}` |
