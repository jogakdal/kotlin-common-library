...
반복 영역 내 특정 필드를 동적으로 숨기는 고급 시나리오입니다. 기본 사용법은 [기본 예제 - 필드 숨기기](./basic-examples.md#8-필드-숨기기)를 참조하세요.

### 14.1 DIM 모드 -- 비활성화 스타일 적용

DELETE 모드(기본값)는 열을 물리적으로 제거하지만, DIM 모드는 열을 유지하면서 셀 값만 제거하고 비활성화 스타일(회색 배경, 연한 글자색)을 적용합니다.
...
- C열의 헤더와 데이터 셀에 회색 배경이 적용되고, 값은 비어 있습니다
